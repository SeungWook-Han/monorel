...
int create_file_counter=0,
  destroy_file_counter=0,
  open_file_counter=0,
  close_file_counter=0,
  get_next_counter=0,
  get_first_counter=0,
  get_this_counter=0,
  alloc_page_counter=0,
  dirty_page_counter=0,
  unpin_page_counter=0,
  print_error_counter=0,
  set_err_stream_counter=0;
...
manage_many_files()
{
  ...
  for (i=0; i<PF_FTAB_SIZE; i++)
    sprintf(files[i],"tmpfile.ifvega.%d",i);
